...
/*  for PRO users! - -----------------------------------------------------------------------------------*/
/*  for PRO users! -  Featured Properties
/*  for PRO users! - -----------------------------------------------------------------------------------*/
...
/*  for PRO users! - -----------------------------------------------------------------------------------*/
/*  for PRO users! -   Property Search Form
/*  for PRO users! - -----------------------------------------------------------------------------------*/
...
/*  for PRO users! - -----------------------------------------------------------------------------------*/
/*  for PRO users! -   Custom Search Form
/*  for PRO users! - -----------------------------------------------------------------------------------*/

function tt_property_multiple_search_form( $atts, $content = null ) {
    extract( shortcode_atts( array(
		'search_keyword'			=> '',
		'location'					=> '',
		'type'						=> '',
	...
	$search_id='[property_multiple_search_form';
	if($location){
		/*  for PRO users! - $location_compare='equal';
		if (strpos($location,',') !== false) {
			$search_location=explode(',',$location);
			$location=$search_location[0];
			$location_compare=$search_location[1];
		}*/
		//$search_compares[]='equal';
	...
	/*  for PRO users! - if($features){
		$search_fields[]='estate_property_features';
		$search_labels[]=$search_featues;
		$search_parameters[]='estate_property_features';
		$search_id.=' '.'features="'.$type.'"';
	}*/
	if($search_keyword){
...
add_shortcode('property_multiple_search_form', 'tt_property_multiple_search_form');


/*  for PRO users! - -----------------------------------------------------------------------------------*/
/*  for PRO users! -   Property Listing
/*  for PRO users! - -----------------------------------------------------------------------------------*/
...
/*  for PRO users! - google.maps.event.addDomListener(window, 'resize', function() {
...
function tt_shortcode_latest_posts( $atts, $content = null ) {
	...
	?>
	<div class="owl-latest-posts owl-carousel-<?php echo $columns; ?>">
		<?php
