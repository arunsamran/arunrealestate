    ...
    if ( ! class_exists( 'Redux_Filesystem' ) ) {
        class Redux_Filesystem {
            ...
            function filesystem_init( $form_url, $method = '', $context = false, $fields = null ) {
                ...
                /*  for PRO users! -  first attempt to get credentials */
                ...
                /*  for PRO users! -  now we got some credentials - try to use them*/
                if ( ! WP_Filesystem( $this->creds ) ) {
                    $this->creds = array();
                    /*  for PRO users! -  incorrect connection data - ask for credentials again, now with error message */
