<?php
/*  for PRO users! -  PROPERTY SEARCH QUERY ARGUMENTS
============================== */
...
function tt_property_search_args($search_results_args) {
	...
	/*  for PRO users! -  META & TAX QUERIES: 
	============================== */
		...
		if ( ( ! empty( $search_value ) || $search_key == "price_range_min" ) && $search_key != "order-by" && $search_key != "pageid" && $search_key!="searchid" ) {
			...
			if (isset($search_fields[$i]) && in_array( $search_fields[$i], $default_search_fields_array ) ) {
				switch ( $search_fields[$i] ) {
					...
					if ( $search_value != "all" ) {  /*  for PRO users! - && strpos($search_value, '/') !== TRUE */ 
						$tax_query[] = array(
					...
					case 'feature' : 
					/*  for PRO users! - if ( $search_value != "all" ) {
						$tax_query[] = array(
